...
def build_model(device: torch.device) -> Sam:
    ...
    load_checkpoint(model, PRETRAINED_CHECKPOINT, map_location="cpu")
    return model.to(device).train().float()
...
def prepare_loader_batch(
    ...
    if batch_img.shape[0] % 2 != 0:
        raise ValueError(
            f"Batch first dimension must be even for paired SAM views, got {batch_img.shape[0]}"
        )
...
def build_setup_summary(
    ...
    summary_lines = [
        ...
        f"Checkpoint interval: {CHECKPOINT_SAVE_INTERVAL}",
        f"Seed: {SEED}",
    ]
...
def main() -> None:
    ...
    setup_lines = build_setup_summary(
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        save_checkpoint(last_path, epoch, model, optimizer, train_loss, val_loss)
        print(f"[checkpoint] saved last -> {last_path}")

        if epoch % CHECKPOINT_SAVE_INTERVAL == 0:
            interval_path = os.path.join(OUTPUT_DIR, f"epoch_{epoch:03d}.pth")
            save_checkpoint(interval_path, epoch, model, optimizer, train_loss, val_loss)
            print(f"[checkpoint] saved interval -> {interval_path}")
        ...
        if val_loss < best_val_loss:
            ...
            save_checkpoint(best_path, epoch, model, optimizer, train_loss, val_loss)
