...
def canonical_layout(root):
    ...
    return {
        ...
        "registration_venv": root / "runtime/registration-venv",
...
quadra_activate() {{
  local profile="${{1:-}}"
  if [[ "${{profile}}" != "preprocess" && "${{profile}}" != "uae" && "${{profile}}" != "registration" ]]; then
    echo "Usage: source {activation} preprocess|uae|registration" >&2
    return 2
  fi
  ...
  if [[ "${{profile}}" == "registration" ]]; then
    if [[ ! -f "{registration_venv}/bin/activate" ]]; then
      echo "Registration environment is missing." >&2
      return 2
    fi
    source "{registration_venv}/bin/activate"
  elif [[ "${{profile}}" == "preprocess" ]]; then
...
""".format(
        ...
        registration_venv=layout["registration_venv"],
    )
